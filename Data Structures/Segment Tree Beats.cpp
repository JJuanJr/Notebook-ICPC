...
struct STree {
  ...
  STree(vector<ll> &a) : N(sz(a)), T(4 * N), A(a) { build(); }
  ...
  void build(int t = 1, int tl = 0, int tr = -1) {
    if (tr == -1) tr = N - 1;
    T[t].lazy = 0;
    if (tl == tr) {
  ...
  void update_add(int l, int r, ll v, int t = 1, int tl = 0, int tr = -1) {
    if (tr == -1) tr = N - 1;
    if (r < tl || tr < l) { return; }
    if (l <= tl && tr <= r) {
      push_add(t, tl, tr, v);
  ...
  void update_chmin(int l, int r, ll v, int t = 1, int tl = 0, int tr = -1) {
    if (tr == -1) tr = N - 1;
  ...
  void update_chmax(int l, int r, ll v, int t = 1, int tl = 0, int tr = -1) {
    if (tr == -1) tr = N - 1;
  ...
  ll query_sum(int l, int r, int t = 1, int tl = 0, int tr = -1) {
    if (tr == -1) tr = N - 1;
